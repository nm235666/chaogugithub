...
DEFAULT_JOBS: tuple[JobDefinition, ...] = (
    ...
    JobDefinition(
        ...
        name="数据就绪 Agent 盘前检查",
        category="agent",
        schedule_expr="0 7 * * 1-5",
        description="盘前检查行情、评分、估值、资金流、风险情景、财务与流动性汇总缺口；必要时自动补数并产出 ready/degraded/blocked 报告",
        commands=(
            py_cmd(str(ROOT_DIR / "jobs" / "run_data_readiness_job.py"), "--job-key", "data_readiness_daily"),
        ),
    ),
    JobDefinition(
        job_key="macro_regime_agent_daily",
        name="三周期状态 Agent 草案生成",
        category="agent",
        schedule_expr="20 8 * * 1-5",
        description="盘后基于完整市场数据生成三周期状态草案；短周期可高频变化，中长周期需更高确认阈值，等待人工确认后写入正式宏观状态",
        commands=(
            py_cmd(str(ROOT_DIR / "jobs" / "run_macro_regime_job.py"), "--job-key", "macro_regime_agent_daily"),
        ),
    ),
    JobDefinition(
        job_key="strategy_selection_daily",
        name="策略选股 Agent 盘前运行",
        category="agent",
        schedule_expr="20 7 * * 1-5",
        description="在数据就绪 Gate 之后、交易建议 Agent 之前，根据市场状态和策略库生成今日策略候选池",
        commands=(
            py_cmd(str(ROOT_DIR / "jobs" / "run_decision_job.py"), "--job-key", "strategy_selection_daily"),
        ),
    ),
    JobDefinition(
        job_key="decision_trade_advisor_daily",
        name="交易建议 Agent 盘前评估",
        category="agent",
        schedule_expr="30 7 * * 1-5",
        description="开盘前批量评估今日动作清单中需要 Agent 复核的股票，并写入缓存供今日交易台读取",
        commands=(
            py_cmd(str(ROOT_DIR / "jobs" / "run_decision_job.py"), "--job-key", "decision_trade_advisor_daily"),
        ),
    ),
    JobDefinition(
    ...
    JobDefinition(
        job_key="agent.funnel_progress.daily",
        name="漏斗推进 Agent",
        category="agent",
        schedule_expr="50 9 * * 1-5",
        enabled=0,
        description="运行漏斗推进 Agent：扫描漏斗断点、dry-run 预检，并自动执行低风险评分对齐与复盘刷新",
        commands=(
            py_cmd(
                str(ROOT_DIR / "jobs" / "run_agent_worker.py"),
                "--agent-key",
                "funnel_progress_agent",
                "--job-key",
                "agent.funnel_progress.daily",
                "--schedule-key",
                "__CN_DATE__",
                "--trigger-source",
                "schedule",
    ...
    JobDefinition(
        job_key="agent.portfolio_reconcile.daily",
        name="持仓重算 Agent",
        category="agent",
        schedule_expr="5 10 * * 1-5",
        description="扫描已执行订单并生成持仓重算审批包，批准后写入 portfolio_positions",
        commands=(
            py_cmd(
                str(ROOT_DIR / "jobs" / "run_agent_worker.py"),
                "--agent-key",
                ...
                "--job-key",
                "agent.portfolio_reconcile.daily",
                "--schedule-key",
                "__CN_DATE__",
                "--trigger-source",
                "schedule",
    ...
    JobDefinition(
        job_key="agent.portfolio_review.daily",
        name="订单复盘 Agent",
        category="agent",
        schedule_expr="15 10 * * 1-5",
        description="扫描已执行订单并生成 T+N 复盘审批包，批准后写入 portfolio_reviews",
        commands=(
            py_cmd(
                str(ROOT_DIR / "jobs" / "run_agent_worker.py"),
                "--agent-key",
    ...
    JobDefinition(
        job_key="agent.decision_orchestrator.daily",
        name="投研闭环编排 Agent",
        category="agent",
        schedule_expr="25 9 * * 1-5",
        enabled=0,
        description="运行编排 Agent：读取质量记忆和闭环断点，创建低风险漏斗/持仓/复盘子 run",
        commands=(
            py_cmd(
                str(ROOT_DIR / "jobs" / "run_agent_worker.py"),
                "--agent-key",
                ...
                "--job-key",
                "agent.decision_orchestrator.daily",
                "--schedule-key",
                "__CN_DATE__",
                "--trigger-source",
                "schedule",
    ...
    JobDefinition(
        job_key="agent.memory_refresh.daily",
        name="Agent 质量记忆刷新",
        category="agent",
        schedule_expr="45 16 * * 1-5",
        enabled=0,
        description="从订单复盘和失败 Agent run 中沉淀质量记忆",
        commands=(
            py_cmd(
                str(ROOT_DIR / "jobs" / "run_agent_worker.py"),
                "--agent-key",
    ...
    JobDefinition(
        job_key="agent.governance_quality_refresh.daily",
        name="Agent 治理质量评分刷新",
        category="agent",
        schedule_expr="55 16 * * 1-5",
        enabled=0,
        description="从 Agent run、审批、审计和质量记忆刷新治理质量评分",
        commands=(
            py_cmd(
                str(ROOT_DIR / "jobs" / "run_agent_worker.py"),
                "--agent-key",
                ...
                "--job-key",
                "agent.governance_quality_refresh.daily",
                "--schedule-key",
                "__CN_DATE__",
                "--trigger-source",
                "schedule",
