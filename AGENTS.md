...
1. 先读「当前最高优先级」与「Project goals」
2. 再读「Workflow」与「Execution rules」
3. 执行前对照「前端任务默认检查项 / 测试任务默认检查项」
4. 输出前对照「Output format」与「文档新鲜度原则」
5. 涉及细节时按「参考文件」跳转，不在本文件扩展动态信息
...
- [主链路](#主链路)
  ...
  - [Workflow](#workflow)
  - [Task framing template](#task-framing-template)
...
- 若后端变更会影响前端展示，必须优先评估兼容层、兜底字段和改动方案。
- 除非任务明确要求，不跨模块做大规模重构。
...
- 每次先分析受影响文件，再动手。
- 默认先给出简短任务理解、涉及模块、风险点、实施计划，再开始实施。
- 先定义“大方向终局目标”（目标能力、验收标准、边界条件），再拆“小路径演进批次”；禁止只围绕最小闭环做局部优化而偏离终局。
- 每次任务先明确“不可破坏项”（前端主链可用、接口兼容、权限边界），再进入实现。
- 错误处理必须输出可行动信息（缺依赖/缺配置/缺数据/超时），避免仅返回抽象失败码。
- 异步任务默认要求可观测：状态、时间、终态原因、关键执行上下文（如模型/解释器/attempts）。
- 优先稳定主路径（能到、能看、能用），再做表达优化与结构优化。
- 兼容优先于重构：新增能力默认非破坏增量，旧字段/旧路径保留至少一个发布周期。
- 验证采用分层口径：静态检查、构建、服务层 smoke、页面 smoke；缺失项必须显式记录风险。
- 运行环境改动必须可复制：提供 setup 脚本 + 最小自检命令。
- 采用“可交付批次”推进：每轮 2-4 个强相关改动，完成一次集中验证后再进入下一轮。
- 若用户要求分提交单元，则严格按顺序执行。
- 每完成一步，运行对应测试或静态检查。
...
- 未经确认，不要跨模块重构。
- 一次只执行一个可交付批次（2-4 个强相关改动）。
- 每一步结束后汇报改动摘要、验证结果、剩余风险。
- 如果发现范围扩大，立即停止并说明原因。
- 当任务与“前端展示业务”和“测试可用性”冲突时，优先保证前端展示主链路可交付，同时补必要验证。
- 当需求明显不合理、跨模块跨度过大、或单批次无法保证主链路可交付时，允许并要求拒绝直接执行。
- 拒绝时不能只给结论，必须给出可执行的分阶段演进方案：终局目标、阶段边界、每批验收标准、风险与回滚点。
- 若用户坚持一次性执行，需再次明确风险，并至少提供一个“最小可交付批次”替代路径。
